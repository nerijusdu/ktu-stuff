...
let runCode file =
    let tree = Path.Combine(__SOURCE_DIRECTORY__, file) |> parseFromFile
    if tree.IsSome then
        Interpreter.run tree.Value.Statements |> ignore
...
//testCase "TestCases/Console.txt" "TestCases/Console.Res.txt"
runCode "TestCases/FunctionType.txt"
//runCode "TestCases/ThrowIf.txt"
