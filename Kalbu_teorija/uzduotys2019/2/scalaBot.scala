...
object ControlFunction
{
    def forMaster(bot: Bot) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestFood) = analyzeViewAsMaster(bot.view)
        ...
        if(bot.energy > -1000){
            nearestFood match{
                ...
                case Some(relPos) =>
                    ...
                    bot.spawn(unitDelta, "mood" -> "Picker", "target" -> remainder)
            }
        }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            ...
            case "Picker"=> reactAsPicker(bot)
            case "Kamikaze" => reactAsKamikaze(bot)
            case s: String => bot.log("unknown mood: " + s)
        }
    }

    def reactAsPicker(bot: MiniBot){
        val (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestFood) = analyzeViewAsMaster(bot.view)
    ...
    def reactAsKamikaze(bot: MiniBot) {
        val (directionValue, nearestEnemyMaster) = analyzeViewAsKamikaze(bot, bot.view)
    ...
    def analyzeViewAsMaster(view: View) = {
        ...
        var nearestFood: Option[XY] = None
        ...
        (directionValue, nearestEnemyMaster, nearestEnemySlave, nearestFood)
    }
    
    def analyzeViewAsKamikaze(bot: MiniBot, view: View) = {
        val directionValue = Array.ofDim[Double](8)
        var nearestEnemyMaster: Option[XY] = None
